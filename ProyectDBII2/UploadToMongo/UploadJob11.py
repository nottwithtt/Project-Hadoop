...
documents = []
with open('resultadoJob11', 'r') as file:
    # Iterate over each line in the file
    for line in file:
        # Process the line
        clean_text = line.replace(" ", "").replace("\t", "").replace("\n", "")
        separatedStrings = clean_text.split(",")
        document = {'Pais':separatedStrings[0],'Anio':int(separatedStrings[1]),'DiferenciaEdades': float(separatedStrings[2]),
        'Valor': float(separatedStrings[3])}
