...
collection = db['Job10A']

# Open the text file in read mode
documents = []
with open('resultadoJob10A', 'r') as file:
    # Iterate over each line in the file
    for line in file:
        # Process the line
        clean_text = line.replace(" ", "").replace("\t", "").replace("\n", "")
        separatedStrings = clean_text.split(",")
        document = {'Pais':separatedStrings[0],'Anio':int(separatedStrings[1]),'PromedioDeMuertes': float(separatedStrings[2]),
