...
documents = []
with open('resultadoJob17', 'r') as file:
    # Iterate over each line in the file
    for line in file:
        # Process the line
        clean_text = line.replace(" ", "").replace("\t", "").replace("\n", "")
        separatedStrings = clean_text.split(",")
        document = {'Pais':separatedStrings[0],'Quintil':separatedStrings[1],separatedStrings[2]:float(separatedStrings[5]),
        separatedStrings[3]:float(separatedStrings[6]),separatedStrings[4]:float(separatedStrings[7])}
